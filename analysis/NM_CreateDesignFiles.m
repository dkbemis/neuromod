...
switch GLA_fmri_type
    ...
    case 'experiment'
        for r = 1:GLA_subject_data.parameters.num_runs
            createExperimentRunDesignFile(GLA_subject_data.runs(r).trials, ...
...
for t = 1:length(types)
    for c = 1:GLA_subject_data.parameters.num_critical_stim
...
for t = 1:length(trials)
    ...
    for n = 1:length(names)
        if strcmp(names{n}, ...
            [trials(t).parameters.p_l ...
                '_' num2str(trials(t).parameters.cond)])
...
for b = 1:GLA_subject_data.parameters.num_localizer_blocks
    onset = GLA_subject_data.localizer.blocks(b).log_stims(1).log_time;
    
    % NOTE: This last stim is actually the start of the delay between
    % blocks, so it works.
    offset = GLA_subject_data.localizer.blocks(b).log_stims(end).log_time;

    % Get the duration
    duration = offset-onset;

    % And convert onset 
    onset = onset - GLA_subject_data.localizer.first_ttl;
    
    % And add
    for n = 1:length(names)
        if strcmp(names{n}, GLA_subject_data.localizer.blocks(b).log_stims(1).condition)
            onsets{n}(end+1) = onset; %#ok<AGROW>
            durations{n}(end+1) = duration; %#ok<AGROW>
            break;
        end
    ...
    if ~isempty(GLA_subject_data.localizer.blocks(b).params.catch_trial)
        ...
        onsets{3}(end+1) = GLA_subject_data.localizer.blocks(b).params.catch_trial{4} - ...
            GLA_subject_data.localizer.blocks(b).params.catch_trial{3} -...
            GLA_subject_data.localizer.first_ttl; %#ok<AGROW>
        
        % The rt
        durations{3}(end+1) = GLA_subject_data.localizer.blocks(b).params.catch_trial{3}; %#ok<AGROW>
    end
end
